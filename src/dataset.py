...
FOREIGN_PER_USD = {"JPY", "CAD", "CHF", "SEK", "NOK", "CNY"}  # currency per 1 USD
...
def build_features(df: pd.DataFrame, config: Config):
    ...
    NOTE: X[t] and Y[t] both represent day t's data (same row = same date).
    Y is NOT pre-shifted here. FXDataset applies time offset via
    X[idx:idx+L] -> Y[idx+L] to create "past L days features -> next day target".
    """
...
class FXDataset(Dataset):
    ...
    Time offset convention:
        __getitem__(idx) returns (X[idx:idx+L], Y[idx+L])
        - Feature window: days idx to idx+L-1 (L days of history)
        - Target: day idx+L (next day after feature window)
        - No leakage: last feature day (idx+L-1) != target day (idx+L)
    """
    ...
    def __getitem__(self, idx: int):
        # Features: past L days [idx, idx+L-1]
        ...
        # Target: next day [idx+L] (1-day ahead of last feature day)
...
def create_dataloaders(config: Config, macro_mode: str = "real"):
    ...
    X (features) are normalized using train-set statistics.
    Y (targets) are kept as raw returns — hit rate should be sign relative to 0.
    """
    df = load_data(config)
    X_local_base, X_macro, Y = build_features(df, config)

    # Normalize X only (train-set statistics)
...
def normalize_data(X_local: np.ndarray, X_macro: np.ndarray, Y: np.ndarray,
                   train_idx: int = None):
    """
    Normalize features using train set statistics. Y is kept raw.

    Args:
        X_local: [T, N, D] local features
        X_macro: [T, M] macro features
        Y: [T, N] targets (returned as-is, not normalized)
    ...
    # Compute train statistics (X only)
