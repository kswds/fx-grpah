...
class SequenceComponentEncoder(nn.Module):
    def __init__(self, input_dim: int, hidden: int, dropout: float):
        ...
        if self.input_dim > 0:
            ...
            self.score_head = nn.Sequential(nn.Linear(hidden, hidden), nn.GELU(), nn.Dropout(dropout), nn.Linear(hidden, 1))

    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        if self.input_dim == 0 or x.size(-1) == 0:
            b, _, n, _ = x.shape
            zeros = torch.zeros(b, n, self.hidden, device=x.device, dtype=x.dtype)
            return {"repr": zeros, "score": torch.zeros(b, n, device=x.device, dtype=x.dtype)}
        b, l, n, _ = x.shape
        z = self.input_proj(x)
        ...
        return {"repr": pooled, "score": score}
...
class StableRelationalScoreGraphFX(nn.Module):
    def __init__(
        ...
        self.macro_regime_encoder = GlobalSequenceEncoder(int(dims.get("global", 0)), hidden, dropout)
    ...
    def _build_graph(self, h: torch.Tensor, h_global: torch.Tensor, A_prev: Optional[torch.Tensor] = None) -> Dict[str, torch.Tensor]:
        b, n, hdim = h.shape
        ...
        msg = torch.einsum("bij,bjh->bih", A, self.Wv(h))
        ...
        edge_contrib = A * self.rel_value_head(h).squeeze(-1)[:, None, :]
        return {
            "adj": A,
            ...
            "rel_repr": h_rel,
            "rel_score": c_rel,
            "regularization": self.graph_regularization(A, A_prev=A_prev),
        }
    ...
    def forward(self, batch: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        regime_out = self.macro_regime_encoder(x_global)
        ...
        h_global = global_out["repr"] if self.use_macro else torch.zeros(x_local.size(0), self.hidden, device=x_local.device, dtype=x_local.dtype)
        h_regime = regime_out["repr"] if self.use_macro else h_global
        ...
        h = self.node_norm(h)
        graph_out = self._build_graph(h, h_regime, A_prev=A_prev)
