...
RELATIONAL_MODELS = {"oursmain", "foundation_relational"}
...
def parse_args():
    parser = argparse.ArgumentParser(description="Refined all-10 hyperparameter search with top_k fixed at 7")
    ...
    parser.add_argument("--output-dir", default=str(ROOT / "results" / "all10_refine_hparam_search"))
    ...
    parser.add_argument("--models", nargs="+", default=["oursmain"])
    ...
    parser.add_argument("--fixed-top-k", type=int, default=7)

    parser.add_argument("--hidden-grid", nargs="+", type=int, default=[48, 64, 80])
    parser.add_argument("--graph-rank-grid", nargs="+", type=int, default=[8, 12, 16])
    parser.add_argument("--dropout-grid", nargs="+", type=float, default=[0.15, 0.20, 0.25])
    parser.add_argument("--edge-dropout-grid", nargs="+", type=float, default=[0.03, 0.05, 0.08])
    parser.add_argument("--spectral-bound-grid", nargs="+", type=float, default=[0.8, 1.0, 1.2])
    parser.add_argument("--lr-grid", nargs="+", type=float, default=[2e-4, 3e-4, 5e-4])
    parser.add_argument("--weight-decay-grid", nargs="+", type=float, default=[1e-4, 3e-4])
    parser.add_argument("--lambda-dir-grid", nargs="+", type=float, default=[0.08, 0.10, 0.12])
    parser.add_argument("--lambda-rank-grid", nargs="+", type=float, default=[0.05, 0.08, 0.10])
    parser.add_argument("--lambda-smooth-grid", nargs="+", type=float, default=[5e-4, 1e-3, 2e-3])
    parser.add_argument("--lambda-static-grid", nargs="+", type=float, default=[5e-4, 1e-3, 2e-3])
    parser.add_argument("--lambda-sparse-grid", nargs="+", type=float, default=[1e-4, 3e-4, 5e-4])
    parser.add_argument("--lambda-spectral-grid", nargs="+", type=float, default=[5e-4, 1e-3, 2e-3])
    parser.add_argument("--small-return-quantile-grid", nargs="+", type=float, default=[0.30, 0.35, 0.40, 0.45])
    ...
    parser.add_argument("--max-trials-per-model", type=int, default=60, help="Hard cap to keep the refined search tractable")
...
def write_markdown_report(agg_df: pd.DataFrame, args, out_dir: Path) -> None:
    lines = [
        ...
        "",
        f"Fixed top_k: `{args.fixed_top_k}`",
        "",
        "Custom currencies:",
    ...
    for model_name, group in agg_df.groupby("model"):
        ...
        for _, row in ranked.iterrows():
            lines.append(
                "| {trial_id} | {hit_ratio_mean:.4f} | {rmse_mean:.6f} | {pairwise_hit_mean:.4f} | {ic_mean:.4f} | {long_short_sharpe_mean:.4f} | {hidden:.0f} | {graph_rank:.0f} | {dropout:.2f} | {edge_dropout:.2f} | {spectral_bound:.2f} | {lr:.5f} | {lambda_dir:.3f} | {lambda_rank:.3f} | {small_return_quantile:.2f} |".format(
                    **row.to_dict()
                )
            )
        lines.append("")
    (out_dir / "refine_hparam_search_summary.md").write_text("\n".join(lines), encoding="utf-8")
...
def main():
    ...
    for model_name in args.models:
        if model_name not in RELATIONAL_MODELS:
            raise ValueError(f"Only relational models are supported: {model_name}")
        ...
        for trial_config in iter_trial_configs(args):
            trial_counter += 1
            if trial_counter > args.max_trials_per_model:
                break
            ...
            for seed in args.seeds:
                ...
                raw_rows.append(row)
            print(
                f"[done] {trial_id} model={model_name} hidden={trial_config['hidden']} graph_rank={trial_config['graph_rank']} "
                f"dropout={trial_config['dropout']:.2f} lr={trial_config['lr']:.5f} q={trial_config['small_return_quantile']:.2f}"
            )
